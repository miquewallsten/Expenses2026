    ...
    <div className="flex h-full min-h-0 overflow-hidden">
      ...
      <div className="flex w-64 shrink-0 flex-col border-r border-subtle">
        ...
        <div className="flex items-center gap-2 border-b border-subtle px-3 py-2.5">
          <BadgeCheck className="h-3.5 w-3.5 text-tertiary" />
          <span className="text-[11px] font-semibold text-secondary">Approve PO</span>
        </div>
        ...
        {filter === "pending" && visible.length > 0 && (
          <div className="border-b border-subtle bg-warning-muted px-3 py-1.5">
            <span className="text-[10px] font-medium text-warning">{visible.length} pendiente{visible.length !== 1 ? "s" : ""} de aprobación</span>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <p className="px-3 pt-6 text-center text-[10px] text-muted">
            ...
            return (
              ...
              >
                ...
                <div className="mt-1.5 flex items-center justify-between gap-2">
                  ...
                  <span className={`text-[10px] font-medium ${PRIORITY_STYLES[req.priority] ?? "text-muted"}`}>
                    {req.priority}
                  </span>
                </div>
              </button>
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-y-auto">
        {!selected ? (
          <div className="flex h-full items-center justify-center">
            <p className="text-[11px] text-muted">Selecciona una solicitud</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4 p-4">
            ...
            {["submitted", "under_review"].includes(selected.status) && (
              <div className="flex flex-col gap-2 rounded-lg border border-default bg-surface-1 p-3">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-muted">Decisión</p>
                ...
                {!showRejectBox && (
                  <textarea
                    ...
                    rows={2}
                    className="w-full rounded border border-default bg-surface-2 px-2 py-1.5 text-[11px] text-secondary placeholder:text-muted focus:outline-none focus:ring-1 focus:ring-sky-500/40"
                  />
                )}
                ...
                {showRejectBox && (
                  <div className="flex flex-col gap-2">
                    <textarea
                      ...
                      rows={2}
                      className="w-full rounded border border-error bg-surface-2 px-2 py-1.5 text-[11px] text-secondary placeholder:text-muted focus:outline-none focus:ring-1 focus:ring-red-500/50"
                    />
                    <button
            ...
            {selected.status === "approved" && (
              <div className="flex items-center gap-1.5 text-[11px] text-success">
                ...
                {selected.reviewer_notes && <span className="text-tertiary"> — {selected.reviewer_notes}</span>}
              </div>
            )}
