  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-0.5 text-[11px] text-white/35">{t("subtitle")}</p>
      </div>
  ...
  return (
    <div className="rounded-lg border border-amber-500/20 bg-amber-500/[0.04] p-4">
      <div className="flex items-start gap-2">
        <Lock className="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-400/70" />
        <div className="space-y-1">
          <p className="text-[12px] font-semibold text-amber-200/85">{t("allDisabledTitle")}</p>
          <p className="text-[11px] leading-relaxed text-amber-200/60">{t("allDisabledBody")}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="w-56 rounded border border-white/[0.08] bg-zinc-900 px-2.5 py-1 text-[11px] text-white/70 placeholder:text-white/25 outline-none focus:border-indigo-500/40"
        />
        ...
        <button
          ...
          className="ml-auto inline-flex items-center gap-1.5 rounded border border-indigo-500/25 bg-indigo-600/10 px-3 py-1 text-[10px] font-semibold text-indigo-300/80 transition-colors hover:bg-indigo-600/20"
        >
      ...
      {err && (
        <div className="flex items-center gap-1.5 rounded border border-red-500/20 bg-red-950/20 px-3 py-1.5 text-[10px] text-red-300/70">
      ...
      <div className="overflow-hidden rounded-lg border border-white/[0.07] bg-white/[0.015]">
        <table className="w-full text-[11px]">
          <thead>
            <tr className="border-b border-white/[0.06] bg-white/[0.02] text-left text-[9px] font-bold uppercase tracking-widest text-white/30">
          ...
          <tbody className="divide-y divide-white/[0.04]">
            ...
            <tr className="bg-white/[0.015]">
              <td className="px-3 py-1.5">
                <input
                  ...
                  className="w-full rounded border border-white/[0.08] bg-zinc-900 px-2 py-0.5 text-[11px] text-white/70 placeholder:text-white/25 outline-none focus:border-indigo-500/40"
                />
              </td>
              <td className="px-3 py-1.5">
                <input
                  ...
                  <td className="px-3 py-1.5 font-mono text-[10.5px]">
                    {isEditing
                      ? <input
                          ...
                          className="w-full rounded border border-white/[0.08] bg-zinc-900 px-2 py-0.5 text-[11px] text-white/70 outline-none focus:border-indigo-500/40"
                        />
                      : row.code
                    }
                  </td>
                      ...
                      {isEditing ? (
                        <>
                          ...
                          <button
                            ...
                            className="rounded border border-white/[0.08] bg-white/[0.03] p-0.5 text-white/50 hover:text-white/80"
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </>
                        ...
                        <>
                          ...
                          <button
                            ...
                            className="rounded border border-red-500/15 bg-red-500/[0.05] p-0.5 text-red-400/60 hover:bg-red-500/[0.15]"
                          >
            ...
            {!loading && filtered.length === 0 && rows.length > 0 && (
              <tr><td colSpan={3} className="px-3 py-4 text-center text-[11px] italic text-white/30">{t("emptyFiltered")}</td></tr>
            )}
            {!loading && rows.length === 0 && (
              <tr><td colSpan={3} className="px-3 py-6 text-center text-[11px] italic text-white/30">{t("emptyAll")}</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function ImportDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-2xl overflow-hidden rounded-lg border border-white/[0.08] bg-zinc-950 shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3">
          <div className="flex items-center gap-2">
            <FileSpreadsheet className="h-4 w-4 text-indigo-400/60" />
            <h3 className="text-[12px] font-semibold text-white/80">{t("importTitle")}</h3>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded border border-white/[0.08] p-1 text-white/40 hover:text-white/80"
          >
        ...
        <div className="max-h-[70vh] overflow-y-auto p-4 space-y-3">
          ...
          {!preview && !result && (
            <div className="space-y-3">
              <p className="text-[11px] text-white/55">{t("importDesc")}</p>
              <button
                type="button"
                disabled={loading}
                onClick={() => fileInputRef.current?.click()}
                className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-white/[0.12] bg-white/[0.02] px-4 py-10 text-[11px] text-white/55 transition-colors hover:border-indigo-500/30 hover:bg-indigo-500/[0.04] disabled:opacity-40"
              >
                {loading
              ...
              />
              <p className="text-[10px] text-white/30">{t("supportedFormats")}</p>
            </div>
          )}
          ...
          {preview && !result && (
            <div className="space-y-3">
              <div className="flex items-start gap-2 rounded border border-indigo-500/15 bg-indigo-500/[0.05] px-3 py-2">
                <Sparkles className="mt-0.5 h-3 w-3 shrink-0 text-indigo-400/60" />
                <p className="text-[10.5px] leading-relaxed text-indigo-200/70">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-[9px] font-bold uppercase tracking-widest text-white/30">{t("mapCode")}</label>
                  <select
                    value={codeCol}
                    onChange={(e) => setCodeCol(e.target.value)}
                    className="w-full rounded border border-white/[0.08] bg-zinc-900 px-2 py-1 text-[11px] text-white/70 outline-none focus:border-indigo-500/40"
                  >
                    <option value="">—</option>
                ...
                <div>
                  <label className="mb-1 block text-[9px] font-bold uppercase tracking-widest text-white/30">{t("mapName")}</label>
                  <select
                    value={nameCol}
              ...
              <div className="overflow-hidden rounded border border-white/[0.07]">
                <table className="w-full text-[10.5px]">
                  ...
                  <tbody className="divide-y divide-white/[0.04]">
                    {mappedPreview.slice(0, 15).map((r) => (
                      <tr key={r.row} className={r.duplicate ? "text-white/30" : "text-white/70"}>
                        <td className="px-2 py-1 text-white/25">{r.row}</td>
                        <td className="px-2 py-1 font-mono">{r.code ?? "—"}</td>
                        <td className="px-2 py-1">{r.name ?? "—"}</td>
                        <td className="px-2 py-1 text-right">
                          {r.duplicate && (
                            <span className="rounded border border-amber-500/20 bg-amber-500/10 px-1.5 py-0.5 text-[8px] font-semibold uppercase text-amber-300/70">
                              {t("duplicate")}
                            </span>
                          )}
                          {!r.duplicate && r.name && r.code && (
                            <span className="rounded border border-emerald-500/20 bg-emerald-500/10 px-1.5 py-0.5 text-[8px] font-semibold uppercase text-emerald-300/70">
                              {t("ready")}
                            </span>
                          )}
                        </td>
                ...
                {preview.total_rows > 15 && (
                  <p className="border-t border-white/[0.05] bg-white/[0.015] px-2 py-1 text-[9.5px] text-white/30">
          ...
          {result && (
            <div className="space-y-2">
              <div className="rounded border border-emerald-500/20 bg-emerald-950/20 px-3 py-2.5">
                ...
                <p className="mt-0.5 text-[10.5px] text-emerald-200/70">
              ...
              {result.errors.length > 0 && (
                <div className="rounded border border-red-500/20 bg-red-950/20 px-3 py-2">
                  <p className="mb-1 text-[10.5px] font-semibold text-red-300/80">{t("someErrors")}</p>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-2 border-t border-white/[0.06] bg-white/[0.015] px-4 py-2.5">
          {result ? (
            <button
              type="button"
              onClick={onDone}
              className="inline-flex items-center gap-1.5 rounded border border-indigo-500/30 bg-indigo-600/20 px-3 py-1 text-[10px] font-semibold text-indigo-300"
            >
              {t("done")}
            </button>
          ) : (
            <>
              <button
                type="button"
                onClick={onClose}
                className="rounded border border-white/[0.08] bg-white/[0.03] px-3 py-1 text-[10px] text-white/55 hover:text-white/85"
              >
                {t("cancel")}
              </button>
              {preview && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded border border-indigo-500/30 bg-indigo-600/20 px-3 py-1 text-[10px] font-semibold text-indigo-300 disabled:opacity-30"
                >
