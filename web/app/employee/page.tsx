...
function MyWorkShell() {
  ...
  // Desktop: sidebar starts expanded; AI starts open
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [aiCollapsed,      setAiCollapsed]      = useState(false);
  const [aiWidth,          setAiWidth]          = useState(AI_W);
  ...
  const startAiResize = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setAiWidth(Math.min(440, Math.max(200, startW + delta)));
    };
    const onUp = () => {
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-zinc-950 text-white">
      ...
      {/* AI rail — collapsed strip or resizable panel */}
      {aiCollapsed ? (
        <div
          style={{ width: AI_COL_W }}
          className="flex shrink-0 flex-col items-center border-l border-white/[0.07] bg-zinc-950 pt-2"
        ...
        <div
          style={{ width: aiWidth, minWidth: 200, maxWidth: 440 }}
        ...
        >
          ...
          <div
            ...
            className="absolute bottom-0 left-0 top-0 z-10 w-1 cursor-col-resize opacity-0 transition-opacity hover:opacity-100 hover:bg-indigo-500/25"
          />
          <button
            type="button"
