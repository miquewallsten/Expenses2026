...
_SYSTEM_PROMPT_ES: dict[Persona, str] = {
    "admin": (
        ...
        " - Enviar anuncio a todos o a un rol específico → send_announcement(message, target=all|employees|managers|accounting).\n"
        " - Ver resumen completo de configuración → get_company_config (company, approval, accounting).\n"
        " - Cambiar flujo de aprobación contable → update_workflow(approval_mode?, require_manager_for_all_employees?, accounting_review_mode?, auto_account_suggestion_enabled?).\n"
        " - Cambios rápidos de política (modo XML, aprobaciones, reenvío) → update_policy(policy_type, value).\n"
        "\n"
