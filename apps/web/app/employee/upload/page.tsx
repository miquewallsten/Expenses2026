...
export default function EmployeeUploadPage() {
  ...
  const [submissionResult, setSubmissionResult] = useState<{ reportId: number; documentsLinked: number } | null>(null);
    ...
    for (const file of acceptedFiles) {
      ...
      try {
        ...
        if (!validationRes.ok) {
          ...
          throw new Error(`Validation lookup failed: ${responseText}`);
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmissionResult({ reportId: data.report_id, documentsLinked: data.documents_linked });
      setFiles([]);
    } catch {
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
          ...
          <div className="mb-8 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-6 py-6">
            <div className="flex items-start justify-between gap-6">
              <div>
                ...
                <p className="text-sm text-white/55 mb-3">Your validated files were packaged and sent for approval.</p>
                <div className="flex items-center gap-5">
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-widest text-white/35">Report ID</div>
                    <div className="text-sm font-mono text-white/70">#{submissionResult.reportId}</div>
                  </div>
                  <div className="w-px h-8 bg-white/10" />
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-widest text-white/35">Documents linked</div>
                    <div className="text-sm font-mono text-white/70">{submissionResult.documentsLinked}</div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm text-white/65">
                Drag and drop XML invoices, PDFs, and ticket images. Files are
                validated automatically and prepared for submission.
              </p>
            </div>
        ...
        >
          ...
          <div className="mx-auto max-w-2xl">
            ...
            <p className="mt-3 text-sm text-white/55">
              Supports XML invoices, PDFs, and ticket images. Multiple files are
              supported.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {files.map((file) => {
            ...
            const isBlocked = file.status === "error";
            const needsReview = file.status === "warning";
            const summaryPill = isValid
              ? { label: "Ready", cls: "bg-emerald-500/20 text-emerald-300 border-emerald-500/40" }
              : isBlocked
              ? { label: "Blocked", cls: "bg-red-500/20 text-red-300 border-red-500/40" }
              : needsReview
              ? { label: "Review", cls: "bg-amber-500/20 text-amber-300 border-amber-500/40" }
              : null;
            ...
            >
              <div className="mb-3 flex items-start justify-between gap-4">
                ...
                <span
                  className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium capitalize ${badgeClass(
                    file.status
                  )}`}
                >
                  {file.status}
              ...
              {summaryPill && (
                <div className={`mb-4 inline-flex items-center rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-wider ${summaryPill.cls}`}>
                  {summaryPill.label}
                </div>
              )}
                  ...
                  {!file.validationResults.length ? (
                    <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white/50">
                      {file.status === "uploading"
                        ...
                        : file.status === "error"
                        ? (file.errorMessage ?? "Could not process this file.")
                      ...
                      {file.validationResults.map((result, index) => (
                        ...
                        >
                          <div className="mb-2 flex items-center justify-between gap-3">
                            ...
                            <span
                              className={`rounded-full px-2.5 py-1 text-[11px] font-medium capitalize ${validationBadgeClass(
                                result.status
                              )}`}
                            >
                              {result.status}
